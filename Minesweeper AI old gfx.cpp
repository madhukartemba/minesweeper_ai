...
void rectangle_full(int x1, int y1, int x2, int y2, int color=BLACK)
{
	if(disp_board_en == 0){return;}
	int i, j;
...
void createbutton(char* val,int tx, int ty, int sno, int offx=0, int anim = 0, float pcomplete = -1)
{
	...
	rectangle_full(tx, ty, rectanglelen+tx, rectangleh+ty);
	...
	{
		setcolor(YELLOW);
		line(tx+3, ty+rectangleh-3, tx+3 + pcomplete*(rectanglelen-6), ty+rectangleh-3);
		line(tx+3, ty+rectangleh-2, tx+3 + pcomplete*(rectanglelen-6), ty+rectangleh-2);
	}
...
void displayboard(int anim = 0, float pcomplete = -1)
{
	...
	{
		createbutton((char*)"AI Solve", max(min(boardtx+width*cm, screenx)-200, 0), min(boardty+length*cm+3, screeny-30), 0, 1, anim, pcomplete);
		//createbutton((char*)"AI Solve", max(min(boardtx+width*cm, screenx)-150, 0), min(boardty+length*cm+3, screeny-25), 0, 0, anim, pcomplete);
	}
	if(run_benchmark==0 && anim == 0)
	{
		createbutton((char*)"Exit", max(min(boardtx+width*cm, screenx)-50, 0), min(boardty+length*cm+3, screeny-30), 1, 0);
	}
	setcolor(LIGHTGREEN);
	settextstyle(buttonfont, 0, buttonfontsize);
...
void color_cells(int perimeter[], int pc, int cellstate[], int nbcells[], int nbcellcount)
{
	if(disp_board_en == 0){return;}
	if(showanim==0)
	{
...
void chunk_solve()	//method 3
{
	...
	int tablesize = 2048, tablerowsize = pc+1;
	...
	while(n_cells_processed<pc)
	{
		...
		color_cells(perimeter, pc, cellstate, nbcells, nbcellcount);
...
void benchmark(int ngames)
{
	...
	prepdisplay();
	if(disp_board_en==1){cleardevice();}
	display();
